...
// Pausa los intervalos cuando el componente sale del viewport
function useAnimatedChart(
  points: number,
  range: [number, number],
...
function useCounter(min: number, max: number, interval: number, isActiveRef: RefObject<boolean>) {
...
function usePulse(period: number, isActiveRef: RefObject<boolean>) {
  ...
  useEffect(() => {
    ...
    const animate = (ts: number) => {
      if (isActiveRef.current) {
        ...
        start = null; // resetea al volver al viewport para una animación suave
      }
...
const CattleAuctionAnimation = () => {
  ...
  // IntersectionObserver: pausa animaciones cuando el componente no es visible
  useEffect(() => {
  ...
  const priceData = useAnimatedChart(18, [40, 90], isActiveRef);
  const volumeData = useAnimatedChart(18, [25, 75], isActiveRef);
  const price = useCounter(1_650_000, 2_100_000, 900, isActiveRef);
  const t = usePulse(3000, isActiveRef);
  ...
  return (
    <div ref={containerRef} className="w-full rounded-2xl overflow-hidden shadow-xl border border-nature-200">
      <svg
        viewBox="0 0 800 500"
        xmlns="http://www.w3.org/2000/svg"
        className="w-full block"
        style={{ fontFamily: "'Courier New', monospace" }}
        aria-label="Animación de inteligencia ganadera en acción"
        role="img"
      >
        <defs>
